...
@Component({
  ...
  templateUrl: './updatedrole.component.html',
  styleUrl: './updatedrole.component.css'
})
export class UpdatedroleComponent {
  roleId: any;
  roleData: any = {};
  errorMessage: string = '';
  permissions: any[] = [];
  ...
  async getRoleById() {
    ...
    try {
      let userDataResponse = await this.userService.getRolesById(this.roleId, token);
      ...
      this.roleData.permissions.forEach((permission: any) => this.selectedPermissions.add(permission.id));
    } catch (error: any) {
  ...
  async updateRole() {
    const confirmUpdate = confirm('¿Estás seguro de que quieres actualizar este rol?');
    ...
    try {
      ...
      const res = await this.userService.updateRole(this.roleId, selectedPermissionIds, token);
      console.log(res);
